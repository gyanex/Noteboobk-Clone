...
function mapResponseModalityToGeminiLiveResponseModality(responseModality) {
    return responseModality === schema.ModalityType.TEXT ? genai.Modality.TEXT : responseModality === schema.ModalityType.AUDIO ? genai.Modality.AUDIO : genai.Modality.IMAGE;
}
 ...
 */ async function messageContentDetailToGeminiPart(content, client) {
    ...
    const result = await client.files.upload({
        file: utils.base64ToBlob(content.data, content.mimeType),
        config: {
            mimeType: content.mimeType
        }
    });
    ...
    return genai.createPartFromUri(result.uri, result.mimeType);
}
...
class GeminiLiveSession extends llms.LiveLLMSession {
    constructor(){
...
class GeminiLive extends llms.LiveLLM {
    constructor(init){
        super();
        this.apiKey = init?.apiKey ?? env.getEnv("GOOGLE_API_KEY");
 ...
 */ class Gemini extends llms.ToolCallLLM {
    static{
    ...
    async streamChat(params) {
        ...
        return utils.streamConverter(generator, (response)=>{
            const toolCall = response.functionCalls?.map((call)=>({
                    id: call.id ?? env.randomUUID(),
                    name: call.name,
                    input: call.args
...
const mapImagePart = (imageUrl)=>{
    ...
    const { mimeType, base64: data } = utils.extractDataUrlComponents(imageUrl);
...
class GoogleStudio extends llms.ToolCallLLM {
    static{
    ...
    constructor({ temperature, topP, maxTokens, model, ...params }){
        ...
        this.client = new genai.GoogleGenAI(params);
    }
    get supportToolCall() {
        return SUPPORT_TOOL_CALL_MODELS.includes(this.model);
    }
    ...
    async nonStreamChat(params) {
        ...
        if (params.tools?.length) {
            if (config.responseModalities?.includes(genai.Modality.IMAGE)) {
    ...
    async complete(params) {
        ...
        if (params.stream) {
            ...
            return utils.streamConverter(response, (response)=>{
                return {
                    text: response.text ?? "",
                    raw: response
                };
 ...
 */ class GeminiEmbedding extends embeddings.BaseEmbedding {
    constructor(opts){
        ...
        const apiKey = opts?.apiKey ?? env.getEnv("GOOGLE_API_KEY");
        if (!apiKey) {
            throw new Error("Set Google API Key in GOOGLE_API_KEY env variable");
        }
    ...
    async getTextEmbeddingsBatch(texts, options) {
        return await embeddings.batchEmbeddings(texts, this.getTextEmbeddings.bind(this), this.embedBatchSize, options);
    }
...
Object.defineProperty(exports, "Modality", {
  enumerable: true,
  get: function () { return genai.Modality; }
});
exports.DEFAULT_EMBED_BATCH_SIZE = DEFAULT_EMBED_BATCH_SIZE;
exports.DEFAULT_GEMINI_PARAMS = DEFAULT_GEMINI_PARAMS;
exports.DEFAULT_SAFETY_SETTINGS = DEFAULT_SAFETY_SETTINGS;
exports.GEMINI_EMBEDDING_MODEL = GEMINI_EMBEDDING_MODEL;
exports.GEMINI_MESSAGE_ROLE = GEMINI_MESSAGE_ROLE;
exports.GEMINI_MODEL = GEMINI_MODEL;
exports.GEMINI_MODEL_INFO_MAP = GEMINI_MODEL_INFO_MAP;
exports.Gemini = Gemini;
exports.GeminiEmbedding = GeminiEmbedding;
exports.GeminiLive = GeminiLive;
exports.GeminiLiveSession = GeminiLiveSession;
exports.GoogleStudio = GoogleStudio;
exports.ROLES_FROM_GEMINI = ROLES_FROM_GEMINI;
exports.ROLES_TO_GEMINI = ROLES_TO_GEMINI;
exports.SUPPORT_TOOL_CALL_MODELS = SUPPORT_TOOL_CALL_MODELS;
exports.gemini = gemini;
exports.getGoogleStudioInlineData = getGoogleStudioInlineData;
exports.mapBaseToolToGeminiFunctionDeclaration = mapBaseToolToGeminiFunctionDeclaration;
exports.mapBaseToolToGeminiLiveFunctionDeclaration = mapBaseToolToGeminiLiveFunctionDeclaration;
exports.mapResponseModalityToGeminiLiveResponseModality = mapResponseModalityToGeminiLiveResponseModality;
exports.mergeNeighboringSameRoleMessages = mergeNeighboringSameRoleMessages;
exports.messageContentDetailToGeminiPart = messageContentDetailToGeminiPart;
