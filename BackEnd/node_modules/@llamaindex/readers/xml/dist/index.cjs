...
class XMLReader extends schema.FileReader {
    /**
   ...
   */ constructor({ splitLevel } = {}){
    ...
    /** XMLParser */ async loadDataAsContent(fileContent) {
        ...
        const doc = new xmldom.DOMParser().parseFromString(xmlStr, "application/xml");
   ...
   */ _getLeafNodesUpToLevel(root, level) {
    ...
    _parseElementToDocuments(root) {
        ...
        return nodes.map((node)=>new schema.Document({
