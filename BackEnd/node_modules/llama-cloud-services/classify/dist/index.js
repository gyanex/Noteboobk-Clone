import { uploadFileApiV1FilesPost, createClassifyJobApiV1ClassifierJobsPost, getClassifyJobApiV1ClassifierJobsClassifyJobIdGet, StatusEnum, getClassificationJobResultsApiV1ClassifierJobsClassifyJobIdResultsGet } from '../../api/dist/index.js';
...
async function pollForJobCompletion(jobId, interval = 1, maxIterations = 1800, client = undefined) {
    ...
    while(true){
        ...
        const response = await getClassifyJobApiV1ClassifierJobsClassifyJobIdGet(jobOptions);
        if (!response.response.ok) {
            numIterations++;
        }
        if (typeof response.data != "undefined") {
...
async function getJobResult(jobId, client = undefined, projectId = null, organizationId = null, maxRetriesOnError = 10, retryInterval = 0.5) {
    ...
    while(true){
        ...
        const response = await getClassificationJobResultsApiV1ClassifierJobsClassifyJobIdResultsGet(jobOptions);
        if (!response.response.ok) {
            if ("error" in response) {
