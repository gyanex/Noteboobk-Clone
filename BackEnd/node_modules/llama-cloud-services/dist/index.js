...
import { z } from 'zod';
export { LlamaParseReader } from '../reader/dist/index.js';
import * as extract from '../extract/dist/index.js';
import { classify } from '../classify/dist/index.js';
...
class LlamaCloudRetriever extends BaseRetriever {
    ...
    async pageScreenshotNodesToNodeWithScore(nodes, projectId) {
        ...
        const results = await Promise.all(nodes.map(async (n)=>{
            const { data: presigned } = await generateFilePageScreenshotPresignedUrlApiV1FilesIdPageScreenshotsPageIndexPresignedUrlPost({
                throwOnError: true,
                path: {
                    id: n.node.file_id,
    ...
    async pageFigureNodesToNodeWithScore(nodes, projectId) {
        if (!nodes || nodes.length === 0) return [];
        const results = await Promise.all(nodes.map(async (n)=>{
            const { data: presigned } = await generateFilePageFigurePresignedUrlApiV1FilesIdPageFiguresPageIndexFigureNamePresignedUrlPost({
                throwOnError: true,
                path: {
                    id: n.node.file_id,
                    page_index: n.node.page_index,
...
class LlamaExtractAgent {
    ...
    async extract(filePath = undefined, fileContent = undefined, fileName = undefined, project_id = null, organization_id = null, fromUi = undefined, pollingInterval = 1, maxPollingIterations = 1800, maxRetriesOnError = 10, retryInterval = 0.5) {
        return await extract.extract(this.agent.id, filePath, fileContent, fileName, project_id, organization_id, this.client, fromUi, pollingInterval, maxPollingIterations, maxRetriesOnError, retryInterval);
    }
}
class LlamaExtract {
    ...
    async createAgent(name, dataSchema, config = undefined, project_id = null, organization_id = null, maxRetriesOnError = 10, retryInterval = 0.5) {
        const agent = await extract.createAgent(name, dataSchema, config, project_id, organization_id, this.client, maxRetriesOnError, retryInterval);
    ...
    async getAgent(name = undefined, id = undefined, project_id = null, organization_id = null, maxRetriesOnError = 10, retryInterval = 0.5) {
        const agent = await extract.getAgent(id, name, project_id, organization_id, this.client, maxRetriesOnError, retryInterval);
        if (typeof agent != "undefined") {
            return new LlamaExtractAgent(agent, this.client);
        }
    }
    async deleteAgent(id, maxRetriesOnError = 10, retryInterval = 500) {
        return await extract.deleteAgent(id, this.client, maxRetriesOnError, retryInterval);
    }
    async extract(dataSchema, config = undefined, filePath = undefined, fileContent = undefined, fileName = undefined, project_id = null, organization_id = null, pollingInterval = 1, maxPollingIterations = 1800, maxRetriesOnError = 10, retryInterval = 0.5) {
        return await extract.extractStateless(dataSchema, config, filePath, fileContent, fileName, project_id, organization_id, this.client, pollingInterval, maxPollingIterations, maxRetriesOnError, retryInterval);
    }
}
...
class LlamaClassify {
    ...
    async classify(rules, parsingConfiguration, fileContents = undefined, filePaths = undefined, projectId = null, organizationId = null, pollingInterval = 1, maxPollingIterations = 1800, maxRetriesOnError = 10, retryInterval = 0.5) {
        const result = await classify(rules, parsingConfiguration, fileContents, filePaths, projectId, organizationId, this.client, pollingInterval, maxPollingIterations, maxRetriesOnError, retryInterval);
        return result;
    }
}
