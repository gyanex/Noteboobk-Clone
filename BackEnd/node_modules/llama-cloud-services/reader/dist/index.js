...
import { getEnv, fs as fs$1, path as path$1 } from '@llamaindex/env';
 ...
 */ class LlamaParseReader extends FileReader {
   ...
   */ async getJobResult(jobId, resultType) {
    ...
    async loadData(filePath) {
        if (!filePath) {
            ...
            const data = filePath.startsWith("s3://") || filePath.startsWith("http://") || filePath.startsWith("https://") ? filePath : await fs$1.readFile(filePath);
   ...
   */ async loadJson(filePathOrContent) {
        ...
        try {
            const data = isFilePath ? await fs$1.readFile(filePathOrContent) : filePathOrContent;
            // Create a job for the file.
            jobId = await this.#createJob(data, isFilePath ? path$1.basename(filePathOrContent) : undefined);
            if (this.verbose) {
                console.log(`Started parsing the file under job id ${jobId}`);
            }
                ...
                if ("items" in page && Array.isArray(page.items)) {
                    for(let i = 0; i < page.items.length; i++){
                        if ("type" in page.items[i] && page.items[i].type === "table" && "csv" in page.items[i] && typeof page.items[i].csv === "string" && page.items[i].csv != "") {
                            ...
                            await fs$1.writeFile(savePath, page.items[i].csv);
   ...
   */ async getImages(jsonResult, downloadPath) {
        try {
            ...
            try {
                await fs$1.access(downloadPath);
            } catch  {
                await fs$1.mkdir(downloadPath, {
                    recursive: true
                });
            }
            const images = [];
   ...
   */ async getImagePath(downloadPath, jobId, imageName) {
        return path$1.join(downloadPath, `${jobId}-${imageName}`);
    }
    /**
   ...
   */ async fetchAndSaveImage(imageName, imagePath, jobId) {
        ...
        await fs$1.writeFile(imagePath, new Uint8Array(await blob.arrayBuffer()));
    }
    /**
